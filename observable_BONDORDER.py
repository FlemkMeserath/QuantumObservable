...
variab = [
#["H",    "H",   0.74,     0.37], #standard
#["H",    "H",   0.74,     0.18], #pure
#["H",    "H",   0.83,     0.3],  #Doped
["H",    "H",   0.74,     0.45],  #clathrate
["B",    "H",   1.14,     0.37],
["La",   "H",   2.06,     0.37],
["Pd",   "H",   1.47,     0.37],
["Pt",   "H",   1.40,     0.37],
...
ICOHP_CUT     = float(sys.argv[2])
DISTANCE_CUT  = float(sys.argv[3])
            ...
            for b in range(-3,4):
                for c in range(-3,4):
                    ...
                    for q in range(0,len(variab)):
                        if ddd > 0 and ddd < DISTANCE_CUT:
                            if (variab[q][0] == str(types[numbers[i]-1]) and variab[q][1] == str(types[numbers[j]-1])) or ( variab[q][0] == str(types[numbers[j]-1]) and variab[q][1] == str(types[numbers[i]-1])):
                                if math.exp((variab[q][2]-ddd)/variab[q][3]) > ICOHP_CUT:

                                    count = count + 1 
                                    print( f"{types[numbers[i]-1]:<3} " f"{i+1:<3} " f"{types[numbers[j]-1]:<3} " f"{j+1:<3} "     f"{ddd:<2.5f} " f"{variab[q][2]:<2.5f} " f"{math.exp((variab[q][2]-ddd)/variab[q][3]):<2.6f} " f"{a:<1} " f"{b:<1} " f"{c:<1}")
                                    data[i][0] = data[i][0] - distance[0]/ddd * math.exp((variab[q][2]-ddd)/variab[q][3])
                                    data[i][1] = data[i][1] - distance[1]/ddd * math.exp((variab[q][2]-ddd)/variab[q][3])
                                    data[i][2] = data[i][2] - distance[2]/ddd * math.exp((variab[q][2]-ddd)/variab[q][3])
